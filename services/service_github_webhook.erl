...
-module(service_github_webhook).
-author("Arjan Scherpenisse <arjan@scherpenisse.net>").

-svc_title("Update the site repository from a webhook.").
-svc_needauth(false).
...
process_post(_ReqData, Context) ->
    OurToken = m_config:get_value(mod_github, webhook_token, Context),
    case z_context:get_q("token", Context) of
        ...
        Token ->
            case z_convert:to_binary(Token) =:= OurToken of
                ...
                true ->
                    mod_github:make(Context),
                    {struct, [{ok, "Updating site."}]}
            end
    end.
